import { DebugClose, DebugLog, DebugOpen } from "./debug";
import {
...
for (;;) {
  ...
  const text = Box("--root-text", {
    prefWidth:  sizeParentPct(1, 1),
    prefHeight: sizeText(0, 1),
  ...
  const content = Box("--root-content", {
    prefWidth:  sizeParentPct(1),
    prefHeight: sizeParentPct(0.5, 1),
    background: ColorKind_Primary,
    foreground: ColorKind_Text,
  });
  ...
  const editor = Editor.make("--root-editor");
  ...
  root.add(text, content, topBorder, editor, bottomBorder);
